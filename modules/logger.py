...
def setup_agent_logger(
    log_file: str = "obsidian_agent.log", agent_id: str = None, verbose: bool = False, quiet: bool = False
) -> logging.Logger:
    ...
    # File handler for detailed logging (always DEBUG level)
    ...
    # Console handler - only add if not in quiet mode
    if not quiet:
    ...
    # Log setup completion (only if not quiet)
    if not quiet:
        logger.info("=" * 60)
...
def log_verbose(logger, message: str):
    ...
    if hasattr(logger, 'verbose_mode') and logger.verbose_mode and not getattr(logger, 'quiet_mode', False):
