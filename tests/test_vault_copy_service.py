...
class TestVaultInfoBuilder:
    ...
    def test_builder_reset_to_defaults(self, builder):
        ...
        # Set some values
        builder.vault_exists().as_directory().with_file_count(10).with_md_file_count(5).with_path("/test").with_error("error")
...
class TestVaultCopyService:
    ...
    def test_copy_vault_success(self, mock_path_class, service, mock_source_path, mock_destination_path):
        ...
        with patch.object(service, '_validate_source_path') as mock_validate, \
            ...
            service.logger.info.assert_any_call(f"Successfully copied vault to {mock_destination}")

    @patch('modules.vault_copy_service.Path')
    def test_copy_vault_validation_error(self, mock_path_class, service, mock_source_path, mock_destination_path):
        """Test vault copying with validation error."""
        mock_source = Mock()
        mock_destination = Mock()
        mock_path_class.side_effect = [mock_source, mock_destination]

        with patch.object(service, '_validate_source_path', side_effect=Exception("Source not found")):
            with pytest.raises(Exception, match="Failed to copy vault from .* to .*: Source not found"):
                service.copy_vault(mock_source_path, mock_destination_path)

            service.logger.error.assert_called_once()

    @patch('modules.vault_copy_service.Path')
    def test_copy_vault_preparation_error(self, mock_path_class, service, mock_source_path, mock_destination_path):
        """Test vault copying with destination preparation error."""
        mock_source = Mock()
        mock_destination = Mock()
        mock_path_class.side_effect = [mock_source, mock_destination]

        with patch.object(service, '_validate_source_path'), \
             patch.object(service, '_prepare_destination', side_effect=Exception("Cannot create directory")):

            with pytest.raises(Exception, match="Failed to copy vault from .* to .*: Cannot create directory"):
                service.copy_vault(mock_source_path, mock_destination_path)

    @patch('modules.vault_copy_service.Path')
    def test_copy_vault_copy_error(self, mock_path_class, service, mock_source_path, mock_destination_path):
        ...
        with patch.object(service, '_validate_source_path'), \
             ...
             patch.object(service, '_perform_copy', side_effect=Exception("Copy failed")):

            with pytest.raises(Exception, match="Failed to copy vault from .* to .*: Copy failed"):
                service.copy_vault(mock_source_path, mock_destination_path)
    ...
    def test_prepare_destination_remove_existing(self, mock_shutil, service):
        ...
        service.logger.info.assert_any_call(f"Removing existing destination: {mock_path}")
    ...
    def test_prepare_destination_mkdir_error(self, service):
        ...
        with pytest.raises(Exception, match="Failed to prepare destination directory: Permission denied"):
            service._prepare_destination(mock_path)
    ...
    def test_prepare_destination_rmtree_error(self, mock_shutil, service):
        ...
        with pytest.raises(Exception, match="Failed to prepare destination directory: Cannot remove directory"):
            service._prepare_destination(mock_path)
    ...
    def test_perform_copy_error(self, mock_shutil, service):
        ...
        with pytest.raises(Exception, match="Failed during copy operation: Copy operation failed"):
            service._perform_copy(mock_source, mock_destination)
