...
class TestVectorStoreManager:
    ...
    def sample_documents(self):
        ...
        return [
            Document(page_content="This is the first document content", metadata={"source": "doc1.md"}),
            Document(page_content="This is the second document content", metadata={"source": "doc2.md"}),
            Document(page_content="This is the third document with much longer content to test chunking behavior", metadata={"source": "doc3.md"}),
        ]
    ...
    def test_init_local_mode(self, mock_get_config, mock_logger, mock_embeddings, mock_config):
    ...
    @patch('modules.vector_store_manager.get_config')
    def test_init_remote_mode(self, mock_get_config, mock_logger, mock_embeddings, mock_config):
        ...
        assert manager.collection_name == "test_collection"  # From config, not parameter
        mock_logger.info.assert_any_call("VectorStoreManager initialized with remote ChromaDB")
        mock_logger.info.assert_any_call("Remote ChromaDB: localhost:8000")

    @patch('modules.vector_store_manager.get_config')
    def test_create_vectorstore_local_success(self, mock_get_config, mock_logger, mock_embeddings, mock_config, sample_documents, sample_chunks):
        ...
        with patch.object(manager, '_split_documents', return_value=sample_chunks) as mock_split, \
    ...
    @patch('modules.vector_store_manager.get_config')
    def test_create_vectorstore_remote_success(self, mock_get_config, mock_logger, mock_embeddings, mock_config, sample_documents, sample_chunks):
    ...
    @patch('modules.vector_store_manager.get_config')
    def test_create_vectorstore_error_handling(self, mock_get_config, mock_logger, mock_embeddings, mock_config, sample_documents):
        ...
        with patch.object(manager, '_split_documents', side_effect=Exception("Split error")):
    ...
    def test_create_local_vectorstore(self, mock_chroma_class, mock_get_config, mock_logger, mock_embeddings, mock_config, sample_chunks):
    ...
    @patch('modules.vector_store_manager.Chroma')
    def test_create_remote_vectorstore(self, mock_chroma_class, mock_chromadb, mock_get_config, mock_logger, mock_embeddings, mock_config, sample_chunks):
    ...
    @patch('modules.vector_store_manager.get_config')
    def test_create_remote_vectorstore_missing_host_port(self, mock_get_config, mock_logger, mock_embeddings, mock_config, sample_chunks):
        ...
        with pytest.raises(ValueError, match="Remote ChromaDB host and port must be specified"):
            manager._create_remote_vectorstore(sample_chunks)

    @patch('modules.vector_store_manager.get_config')
    def test_load_existing_vectorstore_local_success(self, mock_get_config, mock_logger, mock_embeddings, mock_config):
        ...
        with patch.object(manager, '_load_local_vectorstore', return_value=True) as mock_load:
    ...
    @patch('modules.vector_store_manager.get_config')
    def test_load_existing_vectorstore_remote_success(self, mock_get_config, mock_logger, mock_embeddings, mock_config):
        ...
        with patch.object(manager, '_load_remote_vectorstore', return_value=True) as mock_load:
            result = manager.load_existing_vectorstore()
    ...
    @patch('modules.vector_store_manager.get_config')
    def test_load_existing_vectorstore_error_handling(self, mock_get_config, mock_logger, mock_embeddings, mock_config):
        ...
        with patch.object(manager, '_load_local_vectorstore', side_effect=Exception("Load error")):
            result = manager.load_existing_vectorstore()

            assert result is False
            mock_logger.error.assert_called()
            mock_logger.warning.assert_called_with("Will proceed with creating new vector store")

    @patch('modules.vector_store_manager.get_config')
    @patch('modules.vector_store_manager.Chroma')
    @patch('os.path.exists')
    def test_load_local_vectorstore_exists(self, mock_exists, mock_chroma_class, mock_get_config, mock_logger, mock_embeddings, mock_config):
        ...
        with patch('builtins.print') as mock_print:
            ...
            mock_logger.info.assert_any_call("Found existing vector store, loading from: /test/persist")

    @patch('modules.vector_store_manager.get_config')
    @patch('os.path.exists')
    def test_load_local_vectorstore_not_exists(self, mock_exists, mock_get_config, mock_logger, mock_embeddings, mock_config):
    ...
    @patch('modules.vector_store_manager.chromadb')
    @patch('modules.vector_store_manager.Chroma')
    def test_load_remote_vectorstore_exists(self, mock_chroma_class, mock_chromadb, mock_get_config, mock_logger, mock_embeddings, mock_config):
    ...
    @patch('modules.vector_store_manager.get_config')
    @patch('modules.vector_store_manager.chromadb')
    def test_load_remote_vectorstore_not_exists(self, mock_chromadb, mock_get_config, mock_logger, mock_embeddings, mock_config):
        ...
        assert result is False
        mock_logger.debug.assert_any_call("Collection 'test_collection' not found on remote server")

    @patch('modules.vector_store_manager.get_config')
    def test_load_remote_vectorstore_missing_host_port(self, mock_get_config, mock_logger, mock_embeddings, mock_config):
    ...
    @patch('modules.vector_store_manager.get_config')
    def test_get_vectorstore(self, mock_get_config, mock_logger, mock_embeddings, mock_config):
    ...
    @patch('modules.vector_store_manager.get_config')
    def test_get_connection_info_local(self, mock_get_config, mock_logger, mock_embeddings, mock_config):
    ...
    @patch('modules.vector_store_manager.get_config')
    def test_get_connection_info_remote(self, mock_get_config, mock_logger, mock_embeddings, mock_config):
    ...
    def test_split_documents(self, mock_splitter_class, mock_get_config, mock_logger, mock_embeddings, mock_config, sample_documents, sample_chunks):
        ...
        with patch('builtins.print') as mock_print:
            ...
            mock_print.assert_called_with(f"Created {len(sample_chunks)} document chunks")
            mock_logger.info.assert_any_call(f"Created {len(sample_chunks)} document chunks")

    @patch('modules.vector_store_manager.get_config')
    @patch('modules.vector_store_manager.RecursiveCharacterTextSplitter')
    def test_split_documents_custom_config(self, mock_splitter_class, mock_get_config, mock_logger, mock_embeddings, sample_documents):
    ...
    @patch('modules.vector_store_manager.get_config')
    @patch('modules.vector_store_manager.RecursiveCharacterTextSplitter')
    def test_split_documents_default_config(self, mock_splitter_class, mock_get_config, mock_logger, mock_embeddings, sample_documents):
        """Test document splitting with default configuration when config values are missing."""
        incomplete_config = {"COLLECTION_NAME": "test_collection"}  # Missing CHUNK_SIZE and CHUNK_OVERLAP
